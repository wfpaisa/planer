...
- Cada bloque se identifica por un atributo `data-plane="nombre"` en su elemento raíz. La primera vez que se toca un elemento, el servidor le estampa ese nombre (el propuesto para el bloque o, si no se da, uno derivado del título/id/clase/etiqueta del propio trozo). Una vez nombrado, esa identidad queda fija.
- Para editar, se llama `reemplazar_bloque` mandando **solo el HTML del trozo nuevo**, no la página entera. El servidor usa `HTMLRewriter` (el parser en streaming de Bun) para localizar el único elemento que matchea `[data-plane="nombre"]`, reemplazarlo por el nuevo fragmento, y dejar **todo lo demás byte a byte igual** — el propio código lo dice explícito: un documento que pasa por esta transformación sin coincidencias sale idéntico, doctype, comentarios, scripts y todo.
...
- Si el fragmento llega sin su `data-plane`, el servidor se lo repone sobre el primer elemento del trozo — así el bloque no "nace de nuevo" ni pierde su identidad.
...
En español: un cambio local se hace con las herramientas de bloque; solo cuando cambia la estructura o el layout completo se usa `escribir_pagina`, y al final se explica por qué se hizo. La reescritura completa es la excepción explícita para cambios estructurales, no el camino por defecto.

### El catálogo de estilos que la IA no debe reinventar

Las instrucciones del contrato (`shared/htmlContract.ts`) hablan de "the catalogue": no son componentes por-app ni algo generado en un turno anterior, sino **una sola hoja de estilos real y fija de toda la plataforma** — `/plane/estilos.css`, la misma con la que se dibuja el propio panel de Planer (`server/page/pageStyles.ts`). Botones, cards, tablas, tags, campos de formulario, KPIs, tabs, timeline, modales, etc. — todos ya existen como clases CSS reales, compartidas por todas las apps.

### Cómo se recibe el catálogo de componentes UI
...
Así esa lista "de lo que existe" nunca se desincroniza de lo que realmente hay servido — si mañana se agrega una clase al CSS de la casa, el auditor la reconoce automáticamente, sin tocar este archivo. Con esas dos listas, la herramienta `revisar_errores` (la que se usa para revisar el HTML recién escrito) detecta: colores puestos a mano en vez de `var(--algo)`, variables inventadas que no existen en la hoja real, o piezas reinventadas cuando ya había una clase del catálogo para eso.
...
De una conversación solo se mandan al modelo los diez últimos turnos y hasta cuarenta mil caracteres, y empezar una conversación nueva no arrastra nada de las anteriores: se recibe el HTML que quedó, no las reglas que lo explican. Por eso cada página tiene además una **memoria**: un texto en markdown plano, guardado en el campo `memory` de la colección `pages`, con una regla funcional por viñeta — quién puede hacer qué, qué datos son obligatorios, qué prohíbe el negocio. Es del constructor: se lee y se edita en `Memorias`, dentro de los ajustes de la página (`web/src/components/PagePanel.svelte`), y `publicBundle` la quita antes de servir una app publicada.

La memoria entra en el contexto de cada petición como una sección propia detrás de `## This page` (`server/ai/aiPage/prompts.ts`), así que llega igual en una conversación recién empezada que en una que ya pasó del recorte. Antes de construir, se compara lo pedido con ella: si choca con una regla guardada, el turno se cierra con `preguntar` nombrando la regla, y solo se construye si se confirma.

**Escribir la memoria es una pasada aparte, no una instrucción más del turno principal** (`server/ai/aiPage/memory.ts`). Al cerrar el turno se hace una llamada corta, con su propio texto de sistema, que recibe el intercambio entero — la pregunta anterior si la hubo, lo que se pidió, los pasos realizados y la respuesta — más la memoria de ahora, y devuelve las operaciones que el intercambio deje acordadas: `agregar`, `reemplazar` o `borrar`, cada una sobre **una viñeta nombrada por su texto exacto**. Casi siempre son ninguna; un turno que deja acordado el funcionamiento entero de una pantalla son diez. Nada le pone tope a cuántas, ni le dice cómo repartir las reglas entre viñetas: eso se resuelve con el texto de sistema, que da ejemplos de qué clase de cosas son reglas —quién puede hacer qué, qué datos son obligatorios, qué prohíbe el negocio, los estados y lo que exige cada cambio, cómo se calcula algo, qué nace de qué, qué queda en registro— y da una sola prueba para las dudosas: si la página se reconstruyera mañana sin esa línea, ¿saldría mal? Lo único que no puede hacer es devolver la memoria entera reescrita: en la reescritura número quince el modelo resume de más y borra en silencio una regla que nadie miraba. Se aplican en orden, cada una sobre lo que dejó la anterior, y si el texto nombrado ya no aparece esa operación se descarta sin detener a las demás. La pasada no corre cuando el turno cerró preguntando, cerrando un plan o detenido: ahí no se construyó nada y el intercambio está a medias.

**Una orden directa manda sobre el criterio de la pasada.** Si quien construye pide expresamente que algo se recuerde —«recuerda que…», «agrégalo a las memorias»— se guarda, aunque la pasada no lo habría considerado una regla funcional: la lista de lo que la memoria no guarda existe para que el sistema no adivine solo, no para desautorizar al dueño de la app. La respuesta **incluye aquello que hay que recordar**, sin darlo por guardado antes de que termine la pasada, porque la pasada solo ve el intercambio de ese turno y lo acordado en conversaciones anteriores no le llega de ninguna otra forma. Administrar la memoria —borrar una regla, reescribirla entera, leerla— sigue siendo de la caja de los ajustes. Por eso la sección viaja también con la memoria vacía: sin ella la palabra no está en el contexto, y a quien pide que se recuerde algo la explicación no se termina escribiendo dentro de la propia pantalla.
...
## El panel de IA y el contexto de cada solicitud

Una solicitud escrita mientras hay otra en curso espera en la cola y conserva el modo elegido al enviarla.

Plan y Construir reciben guías distintas. El servidor comprueba la lista de herramientas permitidas antes de ejecutar cada llamada, de modo que Plan no puede escribir aunque el proveedor intente llamar una herramienta de edición. Una pregunta o el cierre de un plan detienen también las demás llamadas de esa respuesta. Un plan pasa a implementado cuando la ejecución termina sin escrituras fallidas ni decisiones pendientes —una lectura fallida no cuenta, y una escritura que falló y luego salió bien sobre el mismo bloque o tabla tampoco—; una respuesta cortada o una ejecución fallida lo deja incompleto. Los avisos se guardan junto al mensaje y sobreviven a recargar.

El contrato completo sigue siendo la referencia para la documentación y el harness. Las solicitudes de página reciben una versión compacta con las restricciones imprescindibles, las reglas guardadas, un índice de tablas y el esquema de las fuentes de la página. `consultar_guia` recupera por temas el catálogo de componentes, los colores, las medidas, el oficio de una pantalla, los datos, las gráficas y los bloques, cada uno en una sola lectura; `consultar_tablas` consulta el esquema vigente. Los archivos antiguos siguen disponibles por referencia, sin reenviar sus muestras en cada solicitud. Una imagen de un turno anterior no se presenta como si sus píxeles hubieran vuelto a enviarse.

El historial conserva intercambios completos y las reglas guardadas permanecen aparte. Si se conoce la ventana del modelo, se estima la entrada (unos 3,5 caracteres latinos por token) y la estimación se corrige en cada ronda con los tokens que cuenta el proveedor. El «Máximo por respuesta» configurado se respeta mientras cabe; solo cuando la conversación ocupa casi toda la ventana se pide lo que queda, nunca menos de 4096 tokens, y si la respuesta se corta por eso el aviso lo dice en lugar de pedir subir el máximo. Al empezar se descartan intercambios antiguos completos hasta dejar libre un cuarto de la ventana para responder. Las lecturas repetidas de una misma herramienta y argumentos se sustituyen por la más reciente solo cuando el espacio aprieta, porque reescribir mensajes anteriores anula la caché del proveedor; se conservan los pares de llamadas y resultados y todos los resultados de escritura. Si aun así no cabe, se avisa y se detiene antes de la siguiente llamada. Sin ventana declarada no se promete un porcentaje ni un límite que se desconoce.
...
**Los iconos se buscan, no se listan.** El modelo no recibe ningún catálogo de iconos: `buscar_iconos` busca con palabras en inglés entre los 6228 nombres de la fuente y devuelve solo nombres que existen. El marco de una página lleva pegado el subconjunto de los más comunes, que se dibuja al instante; si la página nombra otro —como clase `icon-…` o como texto entre comillas—, el panel pega sus reglas y le pasa al marco la fuente completa en binario por mensaje, porque un marco sin origen propio no puede pedirla. Un icono se escribe `<i class="icon icon-<nombre>">`: la fuente tiene un solo estilo, así que no hay otra clase de estilo. Las páginas guardadas con la forma de Hugeicons (`hgi-stroke hgi-<nombre>`, o estilos que la fuente no trae como `hgi-solid`) se traducen al leerlas y al guardarlas (`upgradeIconClasses()`); el documento guardado no se reescribe, porque su huella es la que nombran las versiones. Por eso el contrato exige escribir cada nombre entero: uno armado a trozos no se detecta.
