/**
 * Conversion asistida de una página de bloques a una página de HTML.
 *
 * Las aplicaciones que se armaron con bloques no tienen equivalente
 * automatico: un bloque es configuración y una página es un documento. Lo que
 * si se puede hacer es contarle a la IA que había --que bloques, sobre que
 * tablas y con que columnas-- y pedirle el HTML que hace lo mismo.
 *
 * Aquí no se escribe nada. Se devuelve el HTML propuesto con las tablas que
 * declararia, para que quien construye lo revise antes de reemplazar la
 * página. Guardarlo es el mismo guardado de siempre.
 */
...
/** Tope de HTML que se le pide a la IA para una página entera. */
const MAX_TOKENS = 16_000;

/** Los bloques que guarda una página, si es de las de antes. */
...
/**
 * Las columnas que nombra un filtro. Es texto libre, así que no se
...
export function sourcesFromBlocks(blocks: LegacyBlock[], tables: TableRecord[]): HtmlSource[] {
  ...
  for (const [tableId, names] of wanted) {
    ...
    // Por columna definida y no por columna real: una relación ocupa dos
    // columnas en la base --ella y su valor sin dueno-- pero se declara una
    // sola vez, y el puente entrega las dos piezas por ese único nombre.
...
export interface PageConversion {
  html: string;
  sources: HtmlSource[];
  /** Lo que se convirtio, contado en una línea. */
  resumen: string;
}

/**
 * Le pide a la IA el HTML equivalente a los bloques de una página.
...
export async function convertPageToHtml(opts: {
  ...
  // Sin `audience`: esa regla enseña a leer para quien es la pantalla a partir
  // de las palabras de un pedido, y aquí no hay pedido escrito. Lo que entra es
  // la descripcion de unos bloques que ya existian, con el público que traian
  ...
  // El código de los bloques de HTML viaja entero: es lo único que ya estaba
