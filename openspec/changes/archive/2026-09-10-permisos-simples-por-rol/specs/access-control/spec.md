...
**Reason**: El reparto de filas por rol se retira entero. Quien puede abrir una página alcanza todas las filas de las tablas que esa página declara, y lo que se le muestra de ellas lo decide el HTML de la página.

**Migration**: Los alcances declarados en las tablas existentes se descartan al migrar. Una pantalla que antes enseñaba "lo mío" se consigue ahora filtrando dentro de la página contra la identidad de quien mira. Una tabla que sólo debía ver un rol se protege ahora limitando a ese rol las páginas que la usan.

### Requirement: Leer y escribir se declaran por separado

**Reason**: Sin alcance de filas no hay dos alcances que declarar. Lo único que separa leer de escribir es ahora la sesión: leer no la exige, escribir sí.

**Migration**: Ninguna. Ver el requisito "Guardar exige una cuenta iniciada".

### Requirement: Escalera de acciones

**Reason**: Los cinco peldaños (`nada`, `consultar`, `registrar`, `trabajar`, `administrar`) desaparecen junto con el reparto de filas. Quien tiene sesión puede las cinco órdenes; quien no la tiene, sólo las dos de lectura.

**Migration**: Los peldaños declarados en las tablas existentes se descartan al migrar. Una tabla que sólo debía consultarse se protege ahora no ofreciendo en la página los controles de guardar.

### Requirement: Quien tiene varios roles alcanza la unión

**Reason**: Los roles ya no dan acceso a filas, así que no hay alcances que sumar. Una persona con varios roles sigue teniéndolos todos, y el HTML de la página decide qué le muestra a cada uno.

**Migration**: Ninguna. Una persona con varios roles abre las páginas que admitan cualquiera de ellos.

### Requirement: Una tabla nueva no obliga a configurar nada

**Reason**: Ya no hay nada que configurar en una tabla: el reparto por rol y la columna de dueño de fila desaparecen. El requisito describía el comportamiento por defecto de una pantalla que ya no existe.

**Migration**: Ninguna. La columna de dueño de fila declarada en tablas existentes deja de tener efecto y se descarta al migrar.

### Requirement: El ajuste se resume en español

**Reason**: El resumen describía en español lo declarado en la pantalla de "quién ve cada fila", que se elimina. No queda nada que resumir.

**Migration**: Ninguna.

### Requirement: Ningún rol alcanza la fila de otro

**Reason**: La comprobación automática de aislamiento entre roles pierde su objeto: los roles ya no delimitan filas. Sostener una prueba de una propiedad que el sistema ya no promete daría una garantía falsa.

**Migration**: La prueba se reemplaza por otra que comprueba lo que el sistema sí promete ahora: que una página no abre sus datos a quien no puede abrirla, y que sin sesión no se escribe.
