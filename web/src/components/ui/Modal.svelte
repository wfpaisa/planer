<script lang="ts">
  ...
  import { surfaceIn, surfaceOut, veilIn, veilOut } from "../../lib/motion";
   ...
   * `styles/animations.css`--, pero puestas desde JavaScript: aquel es un
   * `popover` y las suyas las lleva el CSS con `@starting-style`; este es un
   * `<div>` que `{#if open}` monta y desmonta, y sin transicion de Svelte el
   * nodo se va antes de que le de tiempo a irse. Los dos tienen que moverse
   * igual --son el mismo modal para quien lo mira, uno en el panel y otro en
   * lo publicado-- y por eso las cuatro salen de `lib/motion.ts`, que lee las
   * duraciones de los mismos tokens que la hoja.
   *
