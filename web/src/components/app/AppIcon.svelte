<!--
  El icono de una aplicacion, con su paleta.

  El color no se elige aparte: es el de la aplicacion. Por eso la pastilla
  lleva su propia paleta --la de la aplicacion, no la del panel-- y de ahi
  ...
  (`compat.css`) tambien se declara ahi, asi que dentro valen igual los
...
<script lang="ts">
  ...
  let {
    ...
    size?: number;
    class?: string;
    /** Si viene, la pastilla se vuelve un boton: cursor, foco y accesible. */
    onclick?: () => void;
...
<style>
  /* La pastilla lleva los colores de la aplicacion, no los del panel. */
  .app-icon {
