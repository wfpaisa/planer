<!--
  ...
  izquierda la salida al panel y la identidad de la aplicación --su icono, su
  nombre, que se cambia escribiendo encima, y sus ajustes--; en el centro las
  dos mitades de la aplicación, la que se ve y la que se guarda; a la derecha lo
  que mira hacia afuera: los ajustes y publicar.

  Publicar solo acompana a las páginas. En la mitad de los datos el botón no
  esta: lo que se toca ahi --las filas-- ya esta afuera en cuanto se guarda, y
  un botón que dice "Publicada" al lado hace pensar que hay que pulsarlo para
  que los datos salgan.

  Quien entra no tiene botón aqui: las personas y sus roles son una tabla mas,
  la primera de "Base de datos, Personas y roles", y se gestionan desde ella.

  Lo de la página abierta no esta aqui: eso vive en el sidebar, y su enlace
  público se copia desde la barra de dirección. Cada panel se abre en su
...
<script lang="ts">
   ...
   * `lib/publishPanel.svelte.ts`): también la abre el candado de la barra de
   * dirección, que esta en otra rama del árbol.
   */
  ...
  // mitad de los datos. Y al entrar en otra aplicación también, que lo abierto
...
<header id="builder-topbar" class="topbar-builder flex items-center gap-2">
  <!--
    ...
    ventana. A propósito ninguno lleva `min-w-0`: asi ningún grupo puede quedar
    mas estrecho que lo que hay dentro. En una ventana muy pequena el del medio
    se corre de su sitio, que es un centro perdido, en vez de que un botón acabe
    dibujado encima de otro.
  -->
  <div class="topbar-group-left">
    <!--
      El `aria-label` no sobra aunque se lea "Aplicaciones": ese texto se
      esconde en ventanas estrechas y el botón se queda en una flecha muda.
    -->
    <Button
      size="sm"
      onclick={onBack}
    ...
    <!--
      ...
      aplicación-- y se dibujan pegados. Tocarle al icono abre los ajustes: la
  ...
  <!--
    Las dos mitades de la aplicación, en el centro y a la misma altura: ninguna
