 ...
 *
 * Guarda dos cosas: que se esta mirando --el borrador o una version-- y si la
 ...
 * y la vista previa dentro del arbol del editor, asi que con las dos abiertas
 ...
 * desplazamiento donde estaba, y vuelve entera desde el boton de la cinta.
 *
 * Y lo que se esta mirando se guarda aqui porque la lista de versiones no lo
 * puede leer de la ruta: se dibuja debajo de la capa, y ahi la ubicacion que ve
 * es la del fondo. El editor, que si ve la ruta de verdad, lo apunta aqui en
 * cuanto cambia; la lista lo lee para senalar la fila que se esta mirando.
 *
 * Vive fuera del arbol porque quien lo escribe y quien lo lee estan en ramas
 * distintas, sin nada en comun mas que el editor entero. Es el mismo caso que
 * `pagePicker.svelte.ts`.
 */

let aside = $state(false);
/** `borrador`, el id de una version, o vacio si no hay capa abierta. */
let source = $state("");
...
/**
 * Lo apunta el editor, que es el unico que ve la ruta. Al cerrarse la capa
 * llega vacio y nada queda apartado: sin capa delante no hay nada que tapar.
 */
