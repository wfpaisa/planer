 ...
 *
 * Dos de sus columnas no estan en la colección que la cuadricula lee: el correo
 * vive en la cuenta --que es de esta aplicación-- y los roles en el enlace con
 * ella. Aquí se juntan al pintar y se reparten al guardar, que es lo que deja
...
/** Lo que se sabe de una persona fuera de la colección de la aplicación. */
export interface PersonOverlay {
  /** Id del enlace persona-aplicación. Es por donde se escriben los roles. */
  accessId: string;
...
/** La forma en que el servidor devuelve un enlace persona-aplicación. */
interface AccessRow {
  id: string;
  member: string;
  roles?: string[];
...
/** Lo que la aplicación sabe de cada persona fuera de su colección, por cuenta. */
 ...
 * en vez de desaparecer: es mas útil verla vacía que no verla.
 */
 ...
 * hay fila donde poner lo demás.
 */
export async function savePersonRow(opts: {
  appId: string;
  table: TableRecord;
  overlay: Map<string, PersonOverlay>;
  /** La fila que se edita, o nada para una persona que todavía no esta invitada. */
  row: Row | null;
  ...
  if (!row) {
     ...
     * obligatoria --una cédula, un legajo-- no deja crearla vacía para llenarla
     * después: se quedaba sin fila, y lo que llegaba a la pantalla era el 404
     * de aquí abajo, al ir a buscar una fila que nunca existio.
     *
     * Solo lo que trae algo: una columna en blanco no hace falta para que la
     * fila nazca, y un vacío donde va un número o una fecha no lo toma la base.
     * Lo demás se escribe igual un poco mas abajo, como siempre.
     */
    ...
    if (!person) throw new Error("La persona se invitó, pero no se encontró su fila");
 ...
 *
 * Cada una es una petición aparte --su correo esta en la cuenta y sus roles en
 * el enlace con la aplicación, y esa puerta no tiene lote-- así que pegar una
 ...
 * en cuatro va rápido y no se le va de las manos.
 */
const AT_ONCE = 4;
 ...
 * cabe en un lote se resuelve aquí: una petición por persona, en tandas, y un
 ...
 * ya es de otra persona de esta aplicación, el mismo correo dos veces en lo
 * pegado, un rol que la aplicación no tiene. Lo que solo sabe la base --una
...
export async function savePeopleColumns(opts: {
  ...
  // Las opciones de la columna de roles son los roles de la aplicación: los
  // pone `withRoleOptions` al leer las tablas.
  const known = rolesField?.options ?? [];

  /** Quien tiene ya cada correo en esta aplicación. */
  ...
  for (const patch of patches) {
    ...
    if (account && patch.cuenta !== undefined) {
      ...
      if (mail === person.cuenta) {
        ...
        // aquí y la base no acepte lo dice ella, y se cuenta como no entro.
        badMail++;
      } else if (other) {
    ...
    if (rolesField && patch.roles !== undefined) {
      ...
      if (fuera.length) {
        /*
         * Un rol que la aplicación no tiene deja la celda como estaba, entera.
         * El servidor descarta en silencio lo que no esta declarado, así que
         * escribir solo el resto guardaria media celda sin decirlo. Crearlo
         * tampoco: los roles se nombran a mano --o importando la nómina, que
         * pregunta antes-- y un nombre de mas en una hoja de cálculo no es
...
export interface PeopleImportReport {
  ...
  sinCorreo: number;
  fueraCorreos: string[];
  /** Los correos que ya tienen cuenta aquí. Son las filas que se actualizan. */
  existenCorreos: string[];
  /** Los roles que nombra el archivo y la aplicación no tenia. Se crean al importar. */
  rolesNuevos: string[];
  claves: { cuenta: string; clave: string }[];
}

/**
 * El nombre que puede traer la columna de la clave en un archivo de nómina.
 *
 * No es una columna de la tabla y no lo sera nunca: se reconoce aquí, se usa
 ...
 *
 * Lo que la importacion normal escribiria en la colección se separa aquí: el
...
export function buildPeopleRows(
  ...
  return bodies.map((body) => {
    ...
    // una columna, así que se busca en el cuerpo crudo.
...
/**
 * Quitar una fila de personas es borrarle la cuenta de esta aplicación.
 *
 * El servidor se lleva con el acceso lo que esta aplicación sabia de ella y la
 * cuenta con la que entraba aquí. Si esta invitada en otra aplicación, alli
