...
function iconsGuide(): string {
  return `An icon is \`<i class="icon icon-NAME"></i>\`: always those two classes, \`icon\` and \`icon-NAME\`, and nothing else. The font has a single style, outlined; there is no filled or duotone version, so never add a style class such as \`hgi-stroke\`, \`hgi-solid\` or \`icon-solid\` --the icon comes out as a broken box--. The font holds more than six thousand names; none is listed here. Find them with "buscar_iconos" using English keywords --several at once if you need icons for several things-- and use only names it returned. Write each name literally in the document (\`icon-NAME\` or a quoted string with the exact name); a name built by joining text is not loaded. Icons take colour and size from their line (1.2em of it); where an icon leads --a section tile, a feature, an empty state-- give it its own size with the --type-* scale (xl for a tile, 2xl for a feature, 3xl for an empty state), never px. Never emojis, never hand-drawn SVG.`;
}
