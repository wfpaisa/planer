/**
 * La lista de personas de una aplicación, tratada como una tabla mas.
 *
 * Aquí esta lo único que la separa de las demás: que dos de sus columnas no
 * viven en su colección. El correo esta en la cuenta --que es de esta
 * aplicación y de ninguna otra-- y los roles, en el enlace persona-aplicación.
 * La colección propia guarda el enlace a la cuenta y las columnas que ponga el
...
/** Nombre tecnico de la tabla de personas dentro de cada aplicación. */
 ...
 * no se renombra-- así que se repone en cada arranque.
 */
 ...
 * colección de cuentas al guardarla-- y los tres leen este número. El tercero
 ...
 *
 * Sale de aquí y no de cada sitio que la necesita --invitar, importar, migrar--
...
export function newPassword(): string {
  ...
  // las 32 primeras salen una vez mas que las demás. Se descartan los valores
...
/**
 * La identidad con la que una persona entra en una aplicación.
 *
 ...
 * lleva delante la aplicación.
 *
 * Quien entra no la escribe ni la ve: el formulario sigue pidiendo el correo y
 * es la pantalla la que le pone delante la aplicación en la que esta. Ver
 ...
 *
 * Es la que una columna que apunta a personas enseña por defecto, y la única
 * que existe desde el primer dia: las demás --un documento, un código-- llegan
 * cuando la aplicación las añade.
 */
 ...
 *
 * `cuenta` es única dentro de la aplicación --dos invitados suyos no comparten
 ...
 * invitado en otra aplicación, y alli es otra persona con otra clave.
 */
 ...
 * una página publicada, la celda de una columna de persona, el desplegable que
 * la elige-- y sin ella no queda mas que el correo.
 *
 * No lleva marca de sistema porque su valor si vive en la colección de la
 ...
 * editarla. Lo que se rellena en cada fila es de la aplicación, como siempre.
 */
 ...
 * que ocupa cada uno.
 *
 * Una página publicada recibe a quien la mira en un solo nivel
 ...
 * aquí: rechazar el nombre una vez es mas claro que resolver el choque cada vez
 * que se arma la sesión. Ver `design.md` D4 de
 * `relacion-automatica-con-personas`.
 *
 * `nombre` no esta en la lista. También viaja en la sesión, pero no hay choque
 * que evitar: la columna que la tabla trae desde el primer dia es de donde sale
 * ese mismo dato (`PEOPLE_DEFAULT_FIELDS`, `personDisplayName`), así que
 * prohibirla rechazaria la tabla de toda aplicación que ya existe.
 */
export const PEOPLE_RESERVED_NAMES: Record<string, string> = {
  ...
  roles: "los roles que tiene en la aplicación",
};

/**
 * Que ocupa ese nombre en la tabla de personas, o vacío si se puede usar.
 *
 ...
 * tabla de personas, la tarjeta que los crea, las opciones de cada página, la
 * vista previa-- y sin una marca comun cada uno parecia una cosa distinta. Es
 * un nombre de la fuente de iconos (ver `shared/icons.ts`), así que se pasa tal
 * cual a `Icon`.
 */
export const ROLE_ICON = "user-account";

/**
 * Cuantos roles puede tener una aplicación y cuanto mide el nombre de uno.
 *
 * El tope existe porque los roles se pintan como opciones de una celda y como
 * renglones marcables en las opciones de cada página: una lista sin fondo no se
...
/**
 * El rol que toda aplicación define desde que nace, sin que nadie lo escriba.
 *
 * Es el de quien construye cuando mira sus propias páginas, y es donde arranca
 * la vista previa. No se puede quitar desde la pantalla de roles: quitarlo
 * dejaria a `pruneRoles` borrandolo de las páginas que lo tuvieran marcado.
 *
 * Dárselo a una persona invitada no le da nada del panel: el poder de quien
 * construye viene de ser dueno de la aplicación, no del nombre del rol. Ver
 * `design.md` D7.
 */
 ...
 * una página compara contra el nombre guardado, y tener dos formas obligaria a
 * saber cual se usa en cada sitio. Ver `design.md` D6.
 */
export function normalizeRole(name: unknown): string {
  // El guion del final se quita después de cortar por el tope: cortar puede
  // dejar uno donde no lo había.
 ...
 *
 * Lo que se guarda es siempre `normalizeRole`, aquí y en el servidor.
 */
 ...
 * colisionen. Lo que se normaliza a vacío se descarta.
 */
...
/** Las columnas que si viven en la colección de la tabla. */
 ...
 *
 * Es la única asimetria que le queda a esta tabla frente a las demás: dos de
 * sus columnas --el correo y los roles-- no estan en su colección, así que una
 * relación que apunte a ella no las trae con el expand de la base ni las puede
 ...
 * cualquiera en cualquier otra tabla, y alli vive en su colección como todas.
 */
...
/** La tabla de personas de una aplicación, si ya existe. */
...
/**
 * Los roles de la aplicación, como opciones de la columna de roles.
 *
 ...
 * así que todo el que las lea --la cuadricula, exportar la estructura, la
...
/**
 * El aviso de que quitar a alguien se lleva por delante su cuenta aquí.
 *
 * Se dice antes de confirmar porque no tiene vuelta atras. Nombra la cuenta
 * --que es de esta aplicación, y borrarla es dejar a esa persona sin entrada--
 * y nombra también lo que no se toca: si esta invitada en otra aplicación,
 ...
 * columna con la que de verdad se reconoce a la gente --"cédula", "documento",
 * "código"-- casi siempre llega importando la nómina y nace como texto
 ...
 *
 * Sin la tabla de personas a mano --una aplicación muy vieja, a medio
 * arrancar-- se cae a la cuenta, que es la única que siempre existe.
 */
 ...
 * de la aplicación, no en la colección propia-- y las dos formas se aceptan
 * porque columnas de antes de este cambio guardan la segunda.
 *
 * Lo demás sale de `campos`, y sale de ahi aunque coincida con un dato de la
 ...
 *
 * La cuenta solo responde cuando esta aplicación no tiene esa columna, que es
 * lo que pasa mientras su tabla de personas todavía no se ha leido.
 */
 ...
 * eso es su cédula, y una página que la ensenara estaria diciendo el documento
 ...
 *
 * Esa columna no se puede borrar, así que la aplicación siempre la tiene. El
 * respaldo a la cuenta sigue aquí para la fila que todavía no la ha rellenado
